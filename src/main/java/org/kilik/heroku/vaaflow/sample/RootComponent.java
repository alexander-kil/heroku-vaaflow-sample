 ...
 * the License.
 */
package org.kilik.heroku.vaaflow.sample;
...
public class RootComponent extends Div implements LocaleChangeObserver {
    ...
    public RootComponent(@Autowired Greeter greeter,
    	...
    	Label intro = new Label("This is Vaadin Flow (Vaadin 10 Prerelease) Sample in Heroku environment");
    	intro.getElement().getStyle().set("display", "block").set("margin-bottom", "20px");
        ...
        add(intro, greeting, button, link, template);
        add(template);
    }
